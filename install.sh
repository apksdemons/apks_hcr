...
uninstall_service() {
  ...
  rm -f -- "$STATE_PATH"
  printf '%b[OK]%b HCR fue desinstalado. El binario oficial y este manager se conservaron.\n' "$C_GREEN" "$C_RESET"
...
show_menu() {
  ...
  printf '%b              Manager PRO v%s%b\n' "$C_GRAY" "$MANAGER_VERSION" "$C_RESET"
  printf '%b%s%b\n' "$C_GOLD" "$BAR" "$C_RESET"
  printf '  %b[1]%b Instalación rápida HCR Plain :%s  %b[16384 / 8s]%b\n' "$C_CYAN" "$C_RESET" "$DEFAULT_PORT" "$C_GREEN" "$C_RESET"
  printf '  %b[2]%b Instalación personalizada (Plain/TLS/Auto)\n' "$C_CYAN" "$C_RESET"
  printf '  %b[3]%b Cambiar puerto HCR\n' "$C_CYAN" "$C_RESET"
  printf '  %b[4]%b Cambiar transport Plain/TLS/Auto\n' "$C_CYAN" "$C_RESET"
  printf '  %b[5]%b Abrir puerto TCP manualmente\n' "$C_CYAN" "$C_RESET"
  printf '  %b[6]%b Cerrar puerto TCP manualmente\n' "$C_CYAN" "$C_RESET"
  printf '  %b[7]%b Estado del servicio\n' "$C_CYAN" "$C_RESET"
  printf '  %b[8]%b Reiniciar HCR\n' "$C_CYAN" "$C_RESET"
  printf '  %b[9]%b Ver registro HCR\n' "$C_CYAN" "$C_RESET"
  printf ' %b[10]%b Desinstalar HCR\n' "$C_CYAN" "$C_RESET"
  printf '  %b[0]%b Salir\n' "$C_CYAN" "$C_RESET"
  status_line
}
...
change_port() {
  ...
  prompt_read value "Nuevo puerto HCR [actual ${PORT}]: " || return 1
  validate_port "$value" || { fail "Puerto inválido."; return 1; }
  PORT="$((10#$value))"
  [ "$PORT" = "$old" ] && { echo "El puerto ya es $PORT."; return 0; }
  install_service
  firewall_open "$PORT" || true
  printf '%b[OK]%b HCR cambió de TCP %s a TCP %s.\n' "$C_GREEN" "$C_RESET" "$old" "$PORT"
...
restart_hcr() {
  ...
  verify_service_health
  printf '%b[OK]%b HCR reiniciado correctamente.\n' "$C_GREEN" "$C_RESET"
}

show_logs() {
  service_is_installed || { fail "HCR aún no está instalado."; return 1; }
  printf '%bÚltimas 80 líneas de HCR:%b\n' "$C_GOLD" "$C_RESET"
  journalctl -u "${SERVICE_NAME}.service" -n 80 --no-pager || true
}

confirm_uninstall() {
  local ans=""
  prompt_read ans "¿Desinstalar HCR? [s/N]: " || return 1
...
menu_loop() {
  while :; do
    ...
    case "$opt" in
      ...
      5) manual_open_port || true ;;
      6) manual_close_port || true ;;
      7) show_status || true ;;
      8) restart_hcr || true ;;
      9) show_logs || true ;;
      10) confirm_uninstall || true ;;
